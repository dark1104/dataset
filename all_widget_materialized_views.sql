...
-- Top Port Numbers per IPV4 address (can convert to application name)
...
-- Top application's by bytes accumulated per IPV4 address
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_tcp_fin_tcp_rst_sourceIPv4Address_agg
ENGINE = SummingMergeTree()
ORDER BY (tcp_fin, tcp_rst, sourceIPv4Address)
AS
SELECT
    tcp_fin,
    tcp_rst,
...
GROUP BY tcp_fin, tcp_rst, sourceIPv4Address;
